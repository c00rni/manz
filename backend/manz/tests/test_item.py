...
class ItemView(TestCase):
    ...
    def _create_item(self, name, quantity_type):
        return Item.objects.create(name=name, quantity_type=quantity_type)

    def _create_recipe_item(self, recipe, item, quantity):
        return RecipeItem.objects.create(recipe=recipe, item=item, quantity=quantity)

    def _create_recipe(self, title: str, description: str, user: User):
        return Recipe.objects.create(
            title="Scrambled Eggs", description="Beautiful scambled eggs", user=user
        )
    ...
    def setUp(self):
        ...
        self.john_scrambled_eggs_recipe = self._create_recipe(
            "Scrambled Eggs", "Beautiful scambled eggs", self.john_user
        )
        self._create_recipe_item(
            self.john_scrambled_eggs_recipe, butter_item, quantity=10
        )
    ...
    def test_should_list_items_needs_for_john_chocolate_cake(self):
        ...
        self._schedule_recipe(
            start_date, end_date, self.john_scrambled_eggs_recipe, self.john_user
        )
        tomorrow = datetime.now() + timedelta(days=1)

        response = Client().get(
            self.item_url + f"?end_date={tomorrow.strftime("%Y-%m-%dT%H:%M:%SZ")}",
